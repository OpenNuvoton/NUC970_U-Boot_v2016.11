...
#define REG_AHBCLK2     0xB0000234

#define REG_SDIC_SIZE0  0xB0003010
#define REG_SDIC_SIZE1  0xB0003014

#define REG_MFP_GPA_L	0xB0000080
#define REG_MFP_GPA_H	0xB0000084
#define REG_MFP_GPB_L	0xB0000088
#define REG_MFP_GPB_H	0xB000008C
#define REG_MFP_GPC_L	0xB0000090
#define REG_MFP_GPC_H	0xB0000094
#define REG_MFP_GPD_L	0xB0000098
#define REG_MFP_GPD_H	0xB000009C
#define REG_MFP_GPE_L	0xB00000A0
#define REG_MFP_GPE_H	0xB00000A4
#define REG_MFP_GPG_L	0xB00000E8
#define REG_MFP_GPG_H	0xB00000EC
#define REG_MFP_GPH 	0xB00000F0

#define SYS_RSTDEBEN    0xB0000100
#define SYS_RSTDEBCTL	0xB0000104

int N32926_cleanup(void);
...
{
        writel(readl(REG_AHBCLK) | 2, REG_AHBCLK); // APB clk
        writel(readl(REG_CLKDIV3) & 0xFFFF00FF, REG_CLKDIV3); // UART1 clock from XTAL
        writel(readl(REG_CLKDIV4) & 0xFF00FFFF, REG_CLKDIV4); // GPIO clock from XTAL
        writel(readl(REG_APBCLK) | 0x110, REG_APBCLK); // TIMER0 and UART1
	return 0;
}

#ifdef CONFIG_N32926_ETH
extern int n32926_eth_register(void);

int board_eth_init(bd_t *bis)
{
        writel(readl(REG_AHBCLK)  | 0x10000, REG_AHBCLK); // HCLK3
        writel(readl(REG_AHBCLK2) | 0x80, REG_AHBCLK2); // EMAC clk
	//Init multi-function pin for RMII
	writel((readl(REG_MFP_GPG_L) & 0xFF0000FF) | 0x00EEEE00, REG_MFP_GPG_L); // G2~G5 for RMII
#ifdef CONFIG_N32926_EMAC_NO_MDC
    // Management interface (MDC/MDIO pins) not used
        writel((readl(REG_MFP_GPD_L) & 0xFFFFF000) | 0x00000EEE, REG_MFP_GPD_L); // D0~D2 for RMII
        writel((readl(REG_MFP_GPD_H) & 0xFFFF0FFF) | 0x0000E000, REG_MFP_GPD_H); // D11 for RMII
#else
        writel((readl(REG_MFP_GPD_L) & 0xFFFF0000) | 0x0000EEEE, REG_MFP_GPD_L); // D0~D3 for RMII
        writel((readl(REG_MFP_GPD_H) & 0xFFFF00FF) | 0x0000EE00, REG_MFP_GPD_H); // D10~D11 for RMII
#endif
	return(n32926_eth_register());
}
...
{
	writel(0x00000FFF, SYS_RSTDEBCTL);
        writel(0x00000001, SYS_RSTDEBEN);
}
