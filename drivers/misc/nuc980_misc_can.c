...
static uint8_t gu8LockCanIf[4ul][2ul];    /* The chip have 4 CANs. */
...
int32_t CAN_SetRxMsg(CAN_T *tCAN, uint32_t u32MsgNum, uint32_t u32IDType, uint32_t u32ID)
{
    ...
    while(CAN_SetRxMsgObj(tCAN, (uint8_t)u32MsgNum, (uint8_t)u32IDType, u32ID, 1) == 0)
    {
...
int32_t CAN_SetRxMsgAndMsk(CAN_T *tCAN, uint32_t u32MsgNum, uint32_t u32IDType, uint32_t u32ID, uint32_t u32IDMask)
{
    ...
    while(CAN_SetRxMsgObjAndMsk(tCAN, (uint8_t)u32MsgNum, (uint8_t)u32IDType, u32ID, u32IDMask, 1) == 0)
    {
        if(++u32TimeOutCount >= RETRY_COUNTS)
        {
            rev = 0;
...
int32_t CAN_SetMultiRxMsg(CAN_T *tCAN, uint32_t u32MsgNum, uint32_t u32MsgCount, uint32_t u32IDType, uint32_t u32ID)
{
    ...
    for(i = 1ul; i <= u32MsgCount; i++)
    {
        ...
        while(CAN_SetRxMsgObj(tCAN, (uint8_t)u32MsgNum, (uint8_t)u32IDType, u32ID, (uint8_t)u32EOB_Flag) == 0)
        {
            if(++u32TimeOutCount >= RETRY_COUNTS)
            {
                rev = 0;
...
int32_t CAN_Receive(CAN_T *tCAN, uint32_t u32MsgNum, STR_CANMSG_T* pCanMsg)
{
    ...
    {
        rev = CAN_ReadMsgObj(tCAN, (uint8_t)u32MsgNum, 1, pCanMsg);
    }
...
void CAN_init(CAN_T *tCAN)
{
	...
	if(tCAN == CAN0) {
		__raw_writel(__raw_readl(REG_PCLKEN1) | (0x1 << 8), REG_PCLKEN1);

		/* Configure multi function pins to CAN0 */
		#if defined (CONFIG_NUC980_MISC_CAN0_PD_1)
		__raw_writel((__raw_readl(REG_SYS_GPD_MFPL) & 0x00ffffff) | 0x44000000, REG_SYS_GPD_MFPL);
		#endif
	} else if(tCAN == CAN1) {
		__raw_writel(__raw_readl(REG_PCLKEN1) | (0x1 << 9), REG_PCLKEN1);

		/* Configure multi function pins to CAN1 */
		#if defined (CONFIG_NUC980_MISC_CAN1_PD_1)
		__raw_writel((__raw_readl(REG_SYS_GPD_MFPH) & 0x00ffffff) | 0x44000000, REG_SYS_GPD_MFPH);
		#endif
	} else if(tCAN == CAN2) {
