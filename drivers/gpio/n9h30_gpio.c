...
static const struct gpio_port *n9h30_gpio_cla_port(unsigned gpio,
						    int *num)
{
	int group;
...
int gpio_set_value(unsigned gpio, int val)
{
	...
	const struct gpio_port *port =
	    n9h30_gpio_cla_port(gpio, &port_num);
...
int gpio_get_value(unsigned gpio)
{
	int port_num, value;
	const struct gpio_port *port;
	port = n9h30_gpio_cla_port(gpio, &port_num);
	value = 0;
