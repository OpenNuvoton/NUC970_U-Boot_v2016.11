...
static const struct gpio_port *n32926_gpio_cla_port(unsigned gpio,
						    int *num)
{
	int group;
...
{
	int port_num, value;
	const struct gpio_port *port = n32926_gpio_cla_port(gpio, &port_num);
...
int gpio_request(unsigned gpio, const char *label)
{
	...
	if (group < 5)
	{
  	...
  	{
		reg   = (unsigned int)REG_MFP_GPG_L;
  	}
	if (num > 7)
	{
		num -= 8;
