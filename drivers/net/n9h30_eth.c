...
int n9h30_eth_mii_write(uchar addr, uchar reg, ushort val)
{
...
int n9h30_eth_mii_read(uchar addr, uchar reg, ushort *val)
{
...
int n9h30_reset_phy(void)
{
        ...
        n9h30_eth_mii_write(CONFIG_N9H30_PHY_ADDR, MII_BMCR, BMCR_RESET);

        delay = 2000;
        while(delay-- > 0) {
                n9h30_eth_mii_read(CONFIG_N9H30_PHY_ADDR, MII_BMCR, &reg);
                if((reg & BMCR_RESET) == 0)
                        break;
        ...
        n9h30_eth_mii_write(CONFIG_N9H30_PHY_ADDR, MII_ADVERTISE, ADVERTISE_CSMA |
                                                                    ADVERTISE_10HALF |
                                                                    ADVERTISE_10FULL |
                                                                    ADVERTISE_100HALF |
                                                                    ADVERTISE_100FULL);
        ...
        n9h30_eth_mii_read(CONFIG_N9H30_PHY_ADDR, MII_BMCR, &reg);
        n9h30_eth_mii_write(CONFIG_N9H30_PHY_ADDR, MII_BMCR, reg | BMCR_ANRESTART);

        delay = 20000;
        while(delay-- > 0) {
                n9h30_eth_mii_read(CONFIG_N9H30_PHY_ADDR, MII_BMSR, &reg);
        ...
        if(delay == 0) {
                ...
                n9h30_eth_mii_read(CONFIG_N9H30_PHY_ADDR, MII_LPA, &reg);
...
int n9h30_eth_write_hwaddr(struct eth_device *dev)
{
...
int n9h30_eth_init(struct eth_device *dev, bd_t *bis)
{
        // Reset MAC
        ...
        n9h30_eth_write_hwaddr(dev);  // need to reconfigure hardware address 'cos we just RESET emc...

        writel(MCMDR_SPCRC | MCMDR_RXON | MCMDR_EnMDC | MCMDR_TXON, MCMDR);
        writel(0, RSDR);

        return(n9h30_reset_phy());

}



void  n9h30_eth_halt(struct eth_device *dev)
{
        writel(readl(MCMDR) & ~(MCMDR_RXON|MCMDR_TXON), MCMDR);
}

int n9h30_eth_recv (struct eth_device *dev)
{
...
int n9h30_eth_send(struct eth_device *dev, void *packet, int length)
{
...
int n9h30_eth_register(void)
{
        ...
        dev->init = n9h30_eth_init;
        dev->halt = n9h30_eth_halt;
        dev->send = n9h30_eth_send;
        dev->recv = n9h30_eth_recv;
        dev->write_hwaddr = n9h30_eth_write_hwaddr;
