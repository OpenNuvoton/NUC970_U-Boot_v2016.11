...
#if defined(CONFIG_SPI_FLASH_W74M)
    #include "rpmc/rpmc.h"
#endif
...
void spi_rpmc_challenge(struct spi_flash *flash)
{
    uint8_t  au8UID[8];
    uint8_t  au8RootKey[32] = {0};
    uint8_t  au8HMACKey[32] = {0};
    uint8_t  au8InputTag[12] = {0};

    uint8_t  au8HMACMsgData[4];
    uint32_t u32RPMCStatus;
    uint32_t u32RPMCCounter;
    uint64_t u64CurrentTick;

    RPMC_ReadUID(flash, &au8UID[0]);
    printf("flash UID [%02X%02X%02X%02X%02X%02X%02X%02X]\n", au8UID[0], au8UID[1], au8UID[2], au8UID[3], au8UID[4], au8UID[5], au8UID[6], au8UID[7]);

    /* Calculate ROOTKey with UID & ROOTKeyTag by SHA256 */
    RPMC_CreateRootKey(&au8UID[0], sizeof(au8UID), &au8RootKey[0]);

    /* Very Important: You should remove below 'First stage' in your production.*/

    /* First stage */
    /* Initial Rootkey, use first rootkey/counter pair */
    u32RPMCStatus = RPMC_WriteRootKey(flash, CONFIG_RPMC_KEY_INDEX, &au8RootKey[0]);
    if (u32RPMCStatus == AF_REG_STATUS_SUCCESS)
    {
        printf("RPMC_WriteRootKey succeed - 0x%02X [%s]!!\n", u32RPMCStatus, CONFIG_RPMC_ROOTKEY_TAG);
    ...
    /* First stage done: Installed Rootkey into W74M. */
    ...
    au8HMACMsgData[0] = prandom_u32() % 0x100;
    au8HMACMsgData[1] = prandom_u32() % 0x100;
    au8HMACMsgData[2] = prandom_u32() % 0x100;
    au8HMACMsgData[3] = prandom_u32() % 0x100;
    ...
    u32RPMCStatus = RPMC_UpdateHMACKey(flash, CONFIG_RPMC_KEY_INDEX, &au8RootKey[0], &au8HMACMsgData[0], &au8HMACKey[0]);
    if (u32RPMCStatus == AF_REG_STATUS_SUCCESS)
    {
        printf("RPMC_UpdateHMACKey succeed - 0x%02X!!\n", u32RPMCStatus);
    }
    else
    {
        printf("RPMC_UpdateHMACKey failed - 0x%02X!! Halted!\n", u32RPMCStatus);
        while (1);
    }

    /* Third stage, increase RPMC counter. */
    /* Input tag is send in to RPMC, it could be time stamp, serial number and so on. */
    u64CurrentTick = get_ticks();
    memcpy(&au8InputTag[0], (void *)&u64CurrentTick, 4);

    u64CurrentTick = get_ticks();
    memcpy(&au8InputTag[4], (void *)&u64CurrentTick, 4);

    u64CurrentTick = get_ticks();
    memcpy(&au8InputTag[8], (void *)&u64CurrentTick, 4);

    u32RPMCStatus = RPMC_IncreaseCounter(flash, CONFIG_RPMC_KEY_INDEX, &au8HMACKey[0], &au8InputTag[0]);
    if (u32RPMCStatus == AF_REG_STATUS_SUCCESS)
    {
        // increase counter success
        printf("RPMC_IncreaseCounter succeed - 0x%02X!!\n", u32RPMCStatus);
    }
    else
    {
        // increase counter fail, check datasheet for the error bit
        printf("RPMC_IncreaseCounter failed - 0x%02X!! Halted!\n", u32RPMCStatus);
        while (1);
    }
    ...
    /* Fourth stage, challenge */
    if (RPMC_Challenge(flash, CONFIG_RPMC_KEY_INDEX, &au8HMACKey[0], &au8InputTag[0]) != 0)
    {
        printf("RPMC_Challenge failed!! Halted!!\n");
        while (1);
    }
}
#endif
...
int spi_flash_scan(struct spi_flash *flash)
{
	...
	if ((jedec == 0x0be1) || (jedec == 0x0bf1) ||
	        ...
	        (jedec == 0xb148) || (jedec == 0xa148) || (jedec == 0xd1c8) || (jedec == 0x91c8) ||
		(jedec == 0x51c8) || (jedec == 0xe240) || (jedec == 0x0b11) || (jedec == 0x0b15) || (jedec == 0x7171) ||
		(jedec == 0xf20a) || (jedec == 0xe47f)) {
