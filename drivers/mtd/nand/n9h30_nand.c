...
#include "../../../board/nuvoton/n9h30/register.h"
...
struct n9h30_nand_info *n9h30_nand;

static struct nand_ecclayout n9h30_nand_oob;
...
static void n9h30_layout_oob_table ( struct nand_ecclayout* pNandOOBTbl, int oobsize , int eccbytes )
{
...
static void n9h30_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
...
/* select chip */
static void n9h30_nand_select_chip(struct mtd_info *mtd, int chip)
{
...
static int n9h30_dev_ready(struct mtd_info *mtd)
{
    int ready;
...
static void n9h30_nand_command_lp(struct mtd_info *mtd, unsigned int command, int column, int page_addr)
{
    register struct nand_chip *chip = mtd->priv;

    //debug("n9h30_nand_command_lp, 0x%x\n", command);
...
/*
 * n9h30_nand_read_byte - read a byte from NAND controller into buffer
 * @mtd: MTD device structure
 */
static unsigned char n9h30_nand_read_byte(struct mtd_info *mtd)
{
//    unsigned char val;
//    val = (unsigned char)readl(REG_SMDATA);
//debug("n9h30_nand_read_byte <0x%x>\n", val);
//    return val;
    return ((unsigned char)readl(REG_SMDATA));
}

/*
 * n9h30_nand_write_buf - write data from buffer into NAND controller
 * @mtd: MTD device structure
 * @buf: virtual address in RAM of source
 * @len: number of data bytes to be transferred
 */

static void n9h30_nand_write_buf(struct mtd_info *mtd, const unsigned char *buf, int len)
{
    int i;

    //debug("n9h30_nand_write_buf\n");
...
/*
 * n9h30_nand_read_buf - read data from NAND controller into buffer
 * @mtd: MTD device structure
 * @buf: virtual address in RAM of source
 * @len: number of data bytes to be transferred
 */
static void n9h30_nand_read_buf(struct mtd_info *mtd, unsigned char *buf, int len)
{
    int i;
//debug("n9h30_nand_read_buf <%d>\n", len);
 ...
 */
void n9h30_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
}
 ...
 */
static int n9h30_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
    return 0;
}
 ...
 * isnull:     unused
 */
static int n9h30_nand_correct_data(struct mtd_info *mtd, u_char *dat,
...
static inline int _n9h30_nand_dma_transfer(struct mtd_info *mtd, const u_char *addr, unsigned int len, int is_write)
{
    struct n9h30_nand_info *nand = n9h30_nand;

    //debug("_n9h30_nand_dma_transfer 0x%x, %d, %d\n", addr, len, mtd->writesize);
 ...
 * @buf:        data buffer
 */
//static void n9h30_nand_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf) //CWWeng 2017.2.14
static int n9h30_nand_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
    uint8_t *ecc_calc = chip->buffers->ecccalc;
    uint32_t hweccbytes=chip->ecc.layout->eccbytes;
    register char * ptr=(char *)REG_SMRA0;

    //debug("n9h30_nand_write_page_hwecc\n");
    memset ( (void*)ptr, 0xFF, mtd->oobsize );
    memcpy ( (void*)ptr, (void*)chip->oob_poi,  mtd->oobsize - chip->ecc.total );

    _n9h30_nand_dma_transfer( mtd, buf, mtd->writesize , 0x1);
...
/**
 * n9h30_nand_read_page_hwecc_oob_first - hardware ecc based page write function
 * @mtd:        mtd info structure
 * @chip:       nand chip info structure
 * @buf:        buffer to store read data
 * @page:       page number to read
 */
//static int n9h30_nand_read_page_hwecc_oob_first(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int page) //CWWeng 2017.2.14
static int n9h30_nand_read_page_hwecc_oob_first(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
    ...
    //debug("n9h30_nand_read_page_hwecc_oob_first\n");
    /* At first, read the OOB area  */
    n9h30_nand_command_lp(mtd, NAND_CMD_READOOB, 0, page);
    n9h30_nand_read_buf(mtd, chip->oob_poi, mtd->oobsize);
    ...
    n9h30_nand_command_lp(mtd, NAND_CMD_READ0, 0, page);
//printk("===========> 0x%x, %d, 0x%x\n", p, eccsize, page);
    _n9h30_nand_dma_transfer(mtd, p, eccsize, 0x0);
#if 0
        for(i=0; i<512; i++)
        {
            printk("%02X ",  *(p+i) );
...
/**
 * n9h30_nand_read_oob_hwecc - [REPLACABLE] the most common OOB data read function
 * @mtd:        mtd info structure
 * @chip:       nand chip info structure
 * @page:       page number to read
 * @sndcmd:     flag whether to issue read command or not
 */
//static int n9h30_nand_read_oob_hwecc(struct mtd_info *mtd, struct nand_chip *chip, int page, int sndcmd) //CWWeng 2017.2.14
static int n9h30_nand_read_oob_hwecc(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
    char * ptr=(char *)REG_SMRA0;

    //debug("n9h30_nand_read_oob_hwecc, page %d, %d\n", page, sndcmd);
    /* At first, read the OOB area  */
    ...
    // Third, read data from nand
    n9h30_nand_command_lp(mtd, NAND_CMD_READ0, 0, page);
    _n9h30_nand_dma_transfer(mtd, NULL, mtd->writesize, 0x0);
...
int board_nand_init(struct nand_chip *nand)
{
    ...
    n9h30_nand = kzalloc(sizeof(struct n9h30_nand_info), GFP_KERNEL);
    if (!n9h30_nand)
        return -1;

    mtd=&n9h30_nand->mtd;
    n9h30_nand->chip.controller = &n9h30_nand->controller;
    ...
    nand->cmd_ctrl = n9h30_hwcontrol;
    nand->cmdfunc = n9h30_nand_command_lp;
    nand->dev_ready = n9h30_dev_ready;
    nand->select_chip = n9h30_nand_select_chip;

    nand->read_byte = n9h30_nand_read_byte;
    nand->write_buf = n9h30_nand_write_buf;
    nand->read_buf = n9h30_nand_read_buf;
    //nand->verify_buf = n9h30_verify_buf;
    nand->chip_delay = 50;

    nand->controller = &n9h30_nand->controller;

    nand->ecc.mode      = NAND_ECC_HW_OOB_FIRST;
    nand->ecc.hwctl     = n9h30_nand_enable_hwecc;
    nand->ecc.calculate = n9h30_nand_calculate_ecc;
    nand->ecc.correct   = n9h30_nand_correct_data;
    nand->ecc.write_page= n9h30_nand_write_page_hwecc;
    nand->ecc.read_page = n9h30_nand_read_page_hwecc_oob_first;
    nand->ecc.read_oob  = n9h30_nand_read_oob_hwecc;
    nand->ecc.layout    = &n9h30_nand_oob;
    ...
    //if (nand_scan_ident(&(n9h30_nand->mtd), 1, NULL)) { //CWWeng 2017.2.14
    ...
    switch (mtd->writesize) {
        case 2048:
            ...
            n9h30_nand->eBCHAlgo = 0; /* T4 */
            n9h30_layout_oob_table ( &n9h30_nand_oob, mtd->oobsize, g_i32ParityNum[1][n9h30_nand->eBCHAlgo] );
            break;

        case 4096:
            writel( (readl(REG_SMCSR)&(~0x30000)) + 0x20000, REG_SMCSR);
            n9h30_nand->eBCHAlgo = 1; /* T8 */
            n9h30_layout_oob_table ( &n9h30_nand_oob, mtd->oobsize, g_i32ParityNum[2][n9h30_nand->eBCHAlgo] );
            break;

        case 8192:
            writel( (readl(REG_SMCSR)&(~0x30000)) + 0x30000, REG_SMCSR);
            n9h30_nand->eBCHAlgo = 2; /* T12 */
            n9h30_layout_oob_table ( &n9h30_nand_oob, mtd->oobsize, g_i32ParityNum[3][n9h30_nand->eBCHAlgo] );
            break;

        /* Not support now. */
        case 512:
            //writel( (readl(REG_SMCSR)&(~0x30000)) + 0, REG_SMCSR);
            //n9h30_nand->m_ePageSize = ePageSize_512;
            //break;
    ...
    if ((readl(REG_PWRON) & 0x300) != 0x300) { /* ECC */
	    ...
	    case 0x000: // T12
	        n9h30_nand->eBCHAlgo = 2;
	        break;

	    case 0x100: // T15
	        n9h30_nand->eBCHAlgo = 3;
	        break;

	    case 0x200: // T24
	        n9h30_nand->eBCHAlgo = 4;
	        break;
	...
	switch ((readl(REG_PWRON) & 0xc0)) {
	    case 0x00: // 2KB
	        mtd->writesize = 2048;
	        writel( (readl(REG_SMCSR)&(~0x30000)) + 0x10000, REG_SMCSR);
		mtd->oobsize = g_i32ParityNum[1][n9h30_nand->eBCHAlgo] + 8;
	        n9h30_layout_oob_table ( &n9h30_nand_oob, mtd->oobsize, g_i32ParityNum[1][n9h30_nand->eBCHAlgo] );
	        break;
	    ...
	    case 0x40: // 4KB
	        mtd->writesize = 4096;
	        writel( (readl(REG_SMCSR)&(~0x30000)) + 0x20000, REG_SMCSR);
		mtd->oobsize = g_i32ParityNum[2][n9h30_nand->eBCHAlgo] + 8;
	        n9h30_layout_oob_table ( &n9h30_nand_oob, mtd->oobsize, g_i32ParityNum[2][n9h30_nand->eBCHAlgo] );
	        break;
	    ...
	    case 0x80: // 8KB
	        mtd->writesize = 8192;
	        writel( (readl(REG_SMCSR)&(~0x30000)) + 0x30000, REG_SMCSR);
		mtd->oobsize = g_i32ParityNum[3][n9h30_nand->eBCHAlgo] + 8;
	        n9h30_layout_oob_table ( &n9h30_nand_oob, mtd->oobsize, g_i32ParityNum[3][n9h30_nand->eBCHAlgo] );
	        break;
