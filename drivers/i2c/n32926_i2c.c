...
#define    REG_APBCLK		(CLK_BA+0x008)  /*  APB IP Clock Enable Control Register */

#define    REG_MFP_GPA_H	(GCR_BA+0x084)  /* GPIOA High Byte Multiple Function Control Register */
#define    REG_MFP_GPB_H	(GCR_BA+0x08C)  /* GPIOB High Byte Multiple Function Control Register */
#define    REG_MFP_GPG_L	(GCR_BA+0x0E8)  /* GPIOG Low Byte Multiple Function Control Register */
...
volatile int32_t N32926_I2C_SPEED = 50;
...
int32_t i2cOpen(uint32_t param)
{
	...
	/* Enable engine clock */
	__raw_writel((__raw_readl(REG_APBCLK) | 0x2), REG_APBCLK);
#if 0
	if((uint32_t)param == 0)
 ...
 * Initialization
 */
static void n32926_i2c_init(struct i2c_adapter *adap, int speed, int slaveaddr)
{
	//printf("\n soft_i2c_init  0x%x \n", adap->hwadapnr);
	...
	if (adap->hwadapnr == 0)
	{
		...
		#if defined (CONFIG_ENABLE_N32926_I2C)
		N32926_I2C_SPEED = CONFIG_N32926_I2C_SPEED;
		#endif

		/* Configure multi function pins to I2C */
		#if defined (CONFIG_N32926_I2C_PA)
		__raw_writel((__raw_readl(REG_MFP_GPA_H) & 0xffff00ff) | 0x00002200, REG_MFP_GPA_H);
		#endif
		#if defined (CONFIG_N32926_I2C_PB)
		__raw_writel((__raw_readl(REG_MFP_GPB_H) & 0xf00fffff) | 0x02200000, REG_MFP_GPB_H);
		#endif
		#if defined (CONFIG_N32926_I2C_PG)
		__raw_writel((__raw_readl(REG_MFP_GPG_L) & 0xff0ff0ff) | 0x00400400, REG_MFP_GPG_L);
		#endif
	}
	else
	{
	...
	i2cIoctl(I2CNUM, I2C_IOC_SET_SPEED, N32926_I2C_SPEED, 0);
}

/*-----------------------------------------------------------------------
 * Probe to see if a chip is present.
 */
static int n32926_i2c_probe(struct i2c_adapter *adap, uint8_t addr)
{
	return 1;
}
 ...
 * Read bytes
 */
static int  n32926_i2c_read(struct i2c_adapter *adap, uchar chip, uint addr,
 ...
 * Write bytes
 */
static int  n32926_i2c_write(struct i2c_adapter *adap, uchar chip, uint addr,
			int alen, uchar *buffer, int len)
{
	int i, j;
	//int tmp;
 ...
 */
U_BOOT_I2C_ADAP_COMPLETE(I2CNUM_0, n32926_i2c_init, n32926_i2c_probe,
			 n32926_i2c_read, n32926_i2c_write, NULL/* _set_speed */,
			 CONFIG_N32926_I2C_SPEED, CONFIG_SYS_I2C_SLAVE,
			 0)
